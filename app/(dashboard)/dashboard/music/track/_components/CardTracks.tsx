...
function CardTrack({ track, filteredArtist, onTrackSelect }: { track: Track, filteredArtist: any, onTrackSelect: (track: Track) => void }) {
    if (!filteredArtist) return null;
    return (
        <div className="flex border-separate flex-col px-2 pb-2 justify-between rounded-lg border shadow-sm shadow-black/[0.1] dark:shadow-white/[0.1]">
            <div className="flex flex-col items-center gap-2 p-4">
                <span className="max-w-[200px] line-clamp-2">{filteredArtist.artist_name}</span>
                <span className="max-w-[200px] line-clamp-2">{track.track_name}</span>
                <Image src={filteredArtist.artist_images[0]} alt="image" width={100} height={100} />
            </div>
